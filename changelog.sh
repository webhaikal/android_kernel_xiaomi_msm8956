...
# file.md can work with more data or have more lines then a page with mor data or have more lines then a page wiki
# input variables set the below the rest must be automatic
source_tree="sensei/msm8956"; #path here must be inside home directory
changelog_path_name=CHANGELOG.mkdn #changelog file path/name.extension
source_name="SenseiKernel" #Name to display in changelog.md top before version
# input variables end
...
k=$(expr $i - 1)
	...
	source=$(git log --oneline --after=$After_Date --until=$Until_Date);

	if [ -n "${source##+([:space:])}" ]; then

		echo "$Until_Date" >> $Changelog;
		echo '============' >> $Changelog;
		git log --oneline --after=$After_Date --until=$Until_Date | sed 's/^$/#EL /' | sed 's/^/* /' | sed 's/* #EL //' >> $Changelog
		echo >> $Changelog;
	fi
